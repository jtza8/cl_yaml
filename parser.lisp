...
(defmethod parse-list ((parser parser))
  (with-slots (past-line present-line future-line) parser
    (let ((items '()) (base-indent (indent-level present-line)))
      (do ((item (scan-to-item (content present-line))
                 (scan-to-item (content present-line))))
          ((or (eql item nil)
               (eql future-line nil)
               (< (indent-level present-line) base-indent)))
        (push item items)
        (advance parser)
        (when (> (indent-level present-line) base-indent)
          (push (parse-list parser) items)))
      (nreverse items))))
